...
public interface DefaultChainableDirectedComponent<A extends DefaultChainableDirectedComponentActivation, C extends Serializable>
	 ...
	 */
	A forwardPropagate(NeuronsActivation input, C context);

	default A forwardPropagateChain(NeuronsActivation input, DirectedComponentsContext context) {
	 ...
	 *         hand side of this DirectedComponent.
	 */
	default Stream<A> forwardPropagateChain(Stream<NeuronsActivation> inputs, DirectedComponentsContext context) {
		return inputs.map(n -> forwardPropagateChain(n, context));
	}
}
